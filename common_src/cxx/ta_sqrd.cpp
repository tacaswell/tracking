...
void TA_sqrdisp::add_disp(const Tuplef & displacement,unsigned step)
{
  // check that steps is less than max steps
  if(step > max_step_)
    throw runtime_error("TA_sqrdisp::add_disp step out of range");
  

  // add magnitude
  r_disp_[step-1].add_data_point(displacement.magnitude_sqr());
  // add angle
...
TA_sqrdisp::TA_sqrdisp(unsigned max_step,
                       unsigned r_nbins,
                       float max_r,
                       unsigned t_nbins):
...
TA_sqrdisp::~TA_sqrdisp()
{
}


  
void TA_sqrdisp::output_to_wrapper(utilities::Generic_wrapper & out,
				   const Md_store & md_store)const
{
  ...
  string base_name = format_name("sqrdisp",comp_key);
